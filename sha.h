...
void printBinary(uint8_t *ptr, size_t length)
{
...
void printHex(uint32_t *array, size_t length)
{
    for (size_t i = 0; i < length; i++)
    {
        printf("%08x ", array[i]);
...
// len in bits
uint8_t* _pad(uint8_t* msg, uint8_t* paddedmsg, uint64_t len, uint32_t* blocks)
{
    uint8_t *ptr;
...
void _hash(uint32_t blocks, uint8_t* paddedmsg, uint32_t* digest)
{
    ...
    for (size_t i = 0; i < blocks; i++)
    {
        ...
        for (size_t j = 0; j < 64; j++)
        {
            ...
            uint32_t t1 = h + sig_1 + ch + k[j] + w[j];
